...
impl<'a> TryFrom<&'a str> for Mac {
    ...
    fn try_from(mac: &'a str) -> Result<Self> {
        ...
        Ok(Mac::Specific([u8::from_str_radix(&mac[..2], 16)?,
...
impl<'a> TryFrom<&'a [u8]> for Mac {
    ...
    fn try_from(mac: &'a [u8]) -> Result<Self> {
        if mac.len() != 6 {
            Err(format!("Malformed mac, expected 6 bytes, got {}", mac.len()))?
        }
...
impl TryFrom<Mac> for Mac {
    type Err = Error;
    fn try_from(mac: Mac) -> Result<Mac> {
        Ok(mac)
    }
}
                ...
                Ok(packet) => {
                    ...
                    if packet_time - self.last_time > self.button.debounce &&
                       match self.button.mac {
                           Mac::Any => {
                               AMAZON_MACS.iter().any(|x: &[u8; 3]| {
                                                          &*x == &frame.source()[..x.len()]
                                                      })
                           }
